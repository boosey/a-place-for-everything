...
function processV1Request (request, response) {
  ...
  const actionHandlers = {
    ...
    'input.store.a.thing': () => {
        ...
        if (parameters.thing &&
            parameters.place &&
            parameters.thing.trim().length > 0 &&
            parameters.thing.trim().length > 0) {
            ...
            requestwithpromise(options)
                .then(function (parsedBody) { // parsedBody = 'OK'
                  if (requestSource === googleAssistantRequest) {
      ...
      // Use the Actions on Google lib to respond to Google requests; for other requests use JSON
        ...
        if (parameters.thing) {
            var options = {
                ...
                //       + parameters.thing,
                uri: 'https://us-central1-a-place-for-everything.cloudfunctions.net/api/api/thingsinplaces/find?thing='
                       + parameters.thing,
                headers: {
                    'Authorization': ' Bearer ' + request.body.originalRequest.data.user.accessToken,
                    'User-Agent': 'Google-Actions'
