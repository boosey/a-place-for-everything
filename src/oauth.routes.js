...
const projectName = 'a-place-for-everything';    // can probably get this from the app object

const verifyClientId = 'thingsinplacesgoogleclientid';
const verifyClientSecret = 'thingsinplacesgoogleclientidsecret';
const verifyBaseRedirectURI = 'https://oauth-redirect.googleusercontent.com/r/' + projectName;
const verifyResponseType = 'code';
...
const app_oauth_route = "https://a-place-for-everything.firebaseapp.com/index-oauth";
...
exports.auth = functions.https.onRequest((req, res) => {
  ...
  if (req.query.client_id === verifyClientId &&
      req.query.redirect_uri === verifyBaseRedirectURI &&
      req.query.response_type === verifyResponseType) {
...
exports.userloggedin = functions.https.onRequest((req, res) => {
  ...
  const authCodeExpiration = (Math.floor(Date.now() / 1000)) + (60 * 60); // 1 hour
...
exports.token = functions.https.onRequest((req, res) => {
  ...
  if (req.body.client_id == verifyClientId &&
      req.body.client_secret == verifyClientSecret) {
    ...
    if (req.body.grant_type == grantTypeAuthorizationCode) {
      ...
      try {
        ...
        if (decodedAuthCode.client_id == verifyClientId) {
          ...
          const accessTokenExpiration = Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 365; // 1 year
      ...
      try {
        ...
        if (decodedRefreshToken.client_id == verifyClientId &&
            decodedRefreshToken.token_type == tokenTypeRefresh) {
              var access_token = jwt.sign(
                {
                  client_id: req.body.client_id,
